...
This can be interpreted and run when new code is submitted by a developer into 
...
4. DUAL_SYNTAX: Languages that use 2 separate language commenting syntax. 
...
5. lines of code with trailing comments count as both LOC and a comment.
...
## Maintainability
Having simple methods that are separate and that do not depend on one another 
...
post the final value to the instance variable equivalent.
...
a string literal (' or "), I jump the index to the next occurrence of that apostrophe 
...
This is a little more tricky and would require a separate small function to determine
this. It's as simple as identifying the location of position 7 and determining 
that this line is a comment. Simple, but since it's barely used, it is out of the scope
of this program.
The same goes for Basic, COBOL: REM comment and Fortran: which uses letter C as comment
